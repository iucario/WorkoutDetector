...
def load_checkpoint(model: VisionTransformer,
                    ckpt_path: str,
                    num_frames: int = 16) -> VisionTransformer:
    if ckpt_path.startswith('https'):
        checkpoint = torch.hub.load_state_dict_from_url(ckpt_path,
                                                        map_location='cpu',
                                                        check_hash=True)
    else:
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).reshape(
                -1, num_frames // model.patch_embed.tubelet_size, new_size, new_size,
                embedding_size)
...
def videomae_vit_small_patch16_224(pretrained=False, **kwargs):
...
def videomae_vit_base_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16,
                              embed_dim=768,
                              depth=12,
                              num_heads=12,
...
def videomae_vit_base_patch16_384(pretrained=False, **kwargs):
...
def videomae_vit_large_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16,
                              embed_dim=1024,
                              depth=24,
                              num_heads=16,
...
def videomae_vit_large_patch16_384(pretrained=False, **kwargs):
    model = VisionTransformer(img_size=384,
                              patch_size=16,
                              embed_dim=1024,
                              depth=24,
...
def videomae_vit_large_patch16_512(pretrained=False, **kwargs):
...
def create_model(num_class=2,
    ...
    if checkpoint is not None:
        model = load_checkpoint(model, checkpoint, num_frames)
    return model