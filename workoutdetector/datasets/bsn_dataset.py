...
class VideoDataSet(data.Dataset):
    ...
    def __init__(self, subset="train", temporal_scale:int = 100):
        self.temporal_scale = temporal_scale
        self.temporal_gap = 1. / self.temporal_scale
        self.subset = subset
        self.mode = 'train'
        self.feature_path = 'activitynet_feature_cuhk'
        self.boundary_ratio = 0.1
        self.video_info_path = 'datasets/activitynet/video_info_new.csv'
        self.video_anno_path = 'datasets/activitynet/anet_anno_action.json'
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            match_score_action, match_score_start, match_score_end, _ = self._get_train_label(
    ...
    def _get_base_data(self, index):
        ...
        #                        str(self.temporal_scale) + "/" + video_name + ".csv")
        # video_data = video_df.values[:, :]
        # video_data = torch.Tensor(video_data)
        # video_data = torch.transpose(video_data, 0, 1)
        # video_data.float()
        video_data = torch.randn(self.temporal_scale, 400)
    ...
    def _get_train_label(self, index, anchor_xmin, anchor_xmax):
        ...
        for jdx in range(len(anchor_xmin)):
            match_score_action.append(
                np.max(
                    self._ioa_with_anchors(anchor_xmin[jdx], anchor_xmax[jdx], gt_xmins,
                                           gt_xmaxs)))
        match_score_start = []
        for jdx in range(len(anchor_xmin)):
        ...
        return match_score_action, match_score_start, match_score_end, gt_bbox
