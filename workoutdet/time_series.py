...
class LSTMNet(nn.Module):
    ...
    def __init__(self, input_dim, num_layers, hidden_dim, output_dim, dropout,
                 device):
...
class ConvNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, output_dim, num_blocks,
                 dropout):
...
class LitModel(LightningModule):
    ...
    def __init__(self, cfg: CfgNode):
        ...
        if not hasattr(cfg.model,
                       'checkpoint') or cfg.model.checkpoint is None:
            self.save_hyperparameters()
        self.net = LSTMNet(cfg.model.input_dim, cfg.model.num_layers,
                           cfg.model.hidden_dim, cfg.model.output_dim,
                           cfg.model.dropout, self.device)
    ...
    def test_epoch_end(self, outputs):
        ...
        df = pd.DataFrame(columns=[
            'name', 'action', 'split', 'acc', 'count', 'gt_count', 'reps',
            'gt_reps'
        ])
        obo, mae = 0, 0
        for o in outputs:
...
def reps_to_label(reps, total, class_idx):
    ...
    for start, end in zip(reps[::2], reps[1::2]):
        ...
        y[mid:end] = [class_idx * 2 + 2] * (end - mid
                                            )  # plus 1 because no-class is 0
    return y
...
class TestDataset(Dataset):
    ...
    def __init__(self, cfg: CfgNode):
        test_data = list(
            get_rep_data(cfg.data.anno_path,
                         os.path.expanduser("~/data/RepCount"),
        ...
        for item in test_data:
            ...
            assert len(
                torch.tensor(test_y).shape) == 1, torch.tensor(test_y).shape
            self.data.append({
                'x': tx,
...
def evaluate():
    ...
    for item in ds:
        ...
        assert input_x.shape[0] == item['x'].shape[0], (input_x.shape,
                                                        item['x'].shape)
    ...
    cfg = CfgNode(
        init_dict={
            ...
            'data': {
                ...
                'anno_path': os.path.expanduser(
                    '~/data/RepCount/annotation.csv'),
                'batch_size': 32 * 1,
                'window': 60,
                'stride': 20,
                'normalize': False,
            ...
            'model': {
                ...
                'num_layers': 3,  # LSTM
                'dropout': 0.5,
                'example_input_array': [1, 60, 12
                                        ],  # batch, window, feature_dim
                'checkpoint': None
            },
            'optimizer': {
                'lr': 1e-4,
